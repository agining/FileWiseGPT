...
class ChatBotInterface:
    def __init__(self):
        ...
        # Aynı dosyanın tekrar tekrar işlenmesini engellemek için hafıza
            ...
            if user_input:
                ...
                if st.session_state["use_agentic_rag"]:
                    with st.status("🧠 Agentic RAG devrede, ajanlar planlıyor ve çalışıyor...", expanded=True) as status:
                        ...
                        output = self.chatbot.ask_with_rag_agents(user_input, st.session_state["history"])
                    ...
                    with st.chat_message("assistant", avatar="https://i.hizliresim.com/n38vi8v.png"):
                        response_stream = self.chatbot._query_stream(user_input, st.session_state["history"])
    ...
    def sidebar_operations(self):
        ...
        if uploaded_files:
            ...
            # Sadece yeni bir dosya eklendiyse işlemi yap
            if file_names != st.session_state["processed_files"]:
                ...
                st.rerun() # Ana sayfayı güncelle ve chat arayüzünü aç
        ...
        if openai_api_key and openai_api_key != self.chatbot.openai_api_key:
            ...
            st.rerun() # Ana sayfayı güncelle ve ajan butonlarını aç
